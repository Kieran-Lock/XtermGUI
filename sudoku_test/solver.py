...
class SudokuSolver:
    ...
    def make_guess(self, coordinate: Coordinate, guess: int) -> None:
    ...
    def get_valid_guesses(self, coordinate: Coordinate) -> Iterable[int]:
        ...
        return (i + 1 for i in range(9) if valid_guesses & (1 << i))

    def next(self) -> Coordinate | None:
        for y in range(9):
            for x in range(9):
                coordinate = Coordinate(x, y)
                if not self.board[coordinate]:
                    return coordinate
        return None
